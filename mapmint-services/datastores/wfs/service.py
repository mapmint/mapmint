...
def load(conf,inputs,outputs):
	...
	try: 
		...
		print >> sys.stderr,pl
		print >> sys.stderr,conf["main"]["dataPath"]+"/"+pl[0].upper()+"/"+pl[1]
...
def display(conf,inputs,outputs):
	...
	try:
	 ...
	 <span class="folder"><a href="#">'''+original_dir.split('/')[len(original_dir.split('/'))-2].replace(".txt","")+'''</a></span>'''
		outputs["Result"]["value"]='''
...
'''
			j+=1;
			outputs["Result"]["value"]+='''
	<li id="browseDirectoriesList'''+inputs["type"]["value"]+''':'''+t.replace(".txt","")+'''" class="collapsable"'''
			outputs["Result"]["value"]+='''>'''
			outputs["Result"]["value"]+='''<div class="checker"><span><input type="checkbox" onclick="alert('ok');if(this.checked) Datawarehouse.loadDir(this.value,\''''+inputs["type"]["value"]+'''\');else Datawarehouse.unloadDir(this.value);" value="'''+inputs["type"]["value"]+''':'''+t.replace(".txt","")+'''" name="" style="opacity: 0;" /></span></div>'''
...
def displayJSON(conf,inputs,outputs):
	...
	try:
		...
		return 4
	try:
		outputs["Result"]["value"]='''{ "name": "'''+original_dir.split('/')[len(original_dir.split('/'))-2].replace(".txt","")+'''", "sub_elements": [ '''
	except:
		if original_dir!=default_dir:
			outputs["Result"]["value"]='''{ "name": "'''+original_dir.replace(".txt","")+'''", "sub_elements": [ '''
	...
	for t in tmp:
		...
		try:
			...
			if j>0:
				outputs["Result"]["value"]+=''', '''
			j+=1;
			outputs["Result"]["value"]+='''{"name": "'''+t.replace(".txt","")+'''", "type": "'''+inputs["type"]["value"]+'''"}'''
		except:
			continue
