function importOSMPoints(conf,inputs,outputs){
  var myProcess = new ZOO.Process(conf["main"]["serverAddress"]+"?metapath=osm-tools",'createShpFromOSMP');
...
function importOSMLines(conf,inputs,outputs){
  var myProcess = new ZOO.Process(conf["main"]["serverAddress"]+"?metapath=osm-tools",'createShpFromOSML');
