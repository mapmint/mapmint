function getSymbolChooser4TTF(conf,inputs,outputs){
  ...
  for(i in inputs)
	...
	if(inputs["ttf"]["value"]!="images"){
	    var myProcess = new ZOO.Process(conf["main"]["serverAddress"]+"?metapath=symbol-tools",'getSymbols');
  ...
  var myProcess1 = new ZOO.Process(conf["main"]["serverAddress"]+"?metapath=symbol-tools",'getAllSymbolsForTTF');
