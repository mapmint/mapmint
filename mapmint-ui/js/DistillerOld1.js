...
Distiller=MLayout.extend({
	...
	$.ajax({
	  ...
	  complete: function(xml,status) {
		...
		$.notifyBar({ cls: "success", html: $(xml.responseXML).find("wps\\:LiteralData").text() });
	      ...
	      Distiller.postgisListRefresh($mj("Distiller.pgisrform.stype").value);
	    }
	});
    },
  initializeDbRemoveWindow: function(){
  ...
  initializeDbWindow: function(){
      //alert($('#browser_selected_dsName')[0].value.replace(/browseDirectoriesList/,""));
	  ...
	  complete: function(xml,status) {
		...
		if(dsName!=null && dsType!=null){
		  $.ajax({
			...
			//Distiller.loadDbFormValues(tmp);
  ...
  loadDbFormValues: function(){
      var dbParams=Array("name","dbname","host","port","password","user","stype");
      if(!arguments[0]){
	for(var t in dbParams){
	  if($mj("Distiller.pgisform."+dbParams[t])){
	    if(t!="stype")
	      $mj("Distiller.pgisform."+dbParams[t]).value="";
	    else
	      $mj("Distiller.pgisform."+dbParams[t]).selectedIndex=-1;
	  }
	}
      }else{
	var tmp=arguments[0];
	for(var t in tmp){
	  if($mj("Distiller.pgisform."+t)){
	    $mj("Distiller.pgisform."+t).value=tmp[t];
	  }
	}
      }
    },
  editDataStore: function(){
      ...
      var dsName=$('#browser_selected_dsName')[0].value.replace(/browseDirectoriesList/,"");
      if(dsType=="postgis" || dsType=="mysql"){
	Distiller.initializeDbWindow((dsType=="mysql"?"MySQL":"PostGIS"),
				     dsName);
	Distiller.loadDbFormValues(null);
      }
      else
	...
	if (arguments[0]=='cancel'){
		...
		$.ajax({
			...
			complete: function(xml,status) {
				...
				Distiller.postgisListRefresh($mj("Distiller.pgisform.stype").value);
			}
		});
